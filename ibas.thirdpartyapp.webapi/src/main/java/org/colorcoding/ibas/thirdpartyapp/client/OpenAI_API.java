...
public class OpenAI_API extends WebApp {
	...
	/**
	 * 以 SSE 方式调用 Chat Completions。该方法是新增能力，旧的 completions/execute 调用保持同步。
	 */
	public void streamCompletions(ChatCompletionRequest request, OpenAIStreamListener listener) throws Exception {
		...
		try (ByteArrayOutputStream outputStream = new ByteArrayOutputStream()) {
			serializer.serialize(request, outputStream);
			this.streamPost(new URI(stringBuilder.toString()).normalize().toString(), headers,
					outputStream.toByteArray(), listener);
		}
	}
